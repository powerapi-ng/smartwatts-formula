...
"""
Run smartwatts on a mongodb database that contain 10 hwpc report per target and 11 rapl report
- mongodb
- influxdb
- sensor
"""
...
@pytest.fixture
def database():
    db = gen_base_db_test(DB_URI, 10, with_first_empty=True)
    yield db
...
def check_db():
    ...
    assert c_output.count_documents({}) == int(c_input.count_documents({}) / 4)
...
def test_normal_behaviour(database):
    config = {'verbose': 0,
              ...
              'formula': {'smartwatts': {'disable-cpu-formula': False, 'disable-dram-formula': True, 'cpu-rapl-ref-event': 'RAPL_ENERGY_PKG',
                                          ...
                                          'cpu-error-threshold': 2.0, 'sensor-reports-frequency': 1000, 'learn-min-samples-required': 4,
